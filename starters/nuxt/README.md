...
Your public key is already in `.env`. Nothing else to configure.

> **If you got here by deploying to Vercel**, the key lives in your Vercel
> project settings, not in this repo — the committed `.env` still holds a
> placeholder. The hosted site works. To run it locally, copy
> `NUXT_PUBLIC_MEMBERSTACK_PUBLIC_KEY` out of Vercel's Environment Variables into `.env` first.
...
Everything fictional lives in **`site.config.js`**: the name, the headline, and the list
...
| File | What it does |
|---|---|
| `site.config.js` | All the demo content. Start here. |
| `plugins/memberstack.client.js` | Creates the SDK. `.client` is load-bearing — the SDK needs `window`. |
| `composables/useMember.js` | The shared session, built on Nuxt's `useState`. |
| `middleware/gated.js` | Route protection. Add it with `definePageMeta`. |
| `pages/signup.vue`, `pages/login.vue` | Email + password, Google, and passwordless. |
| `pages/members.vue`, `pages/account.vue` | The two gated pages. |
...
**The session is shared via `useState`**, Nuxt's own cross-component state. Every
caller of `useMember()` gets the same refs, so signing in updates the nav and the
page together. Call `refresh()` after anything that changes who is signed in.
...
Any Node host. Unlike the other starters the key is read at **runtime**, not baked
into the bundle, so whatever you deploy to must have `NUXT_PUBLIC_MEMBERSTACK_PUBLIC_KEY`
set — otherwise the key is empty and every call fails with no obvious cause.

## Before you launch

Test Mode keys start `pk_sb_` and the members you create are free and never
touch your live site. Live keys start `pk_`. Swap `NUXT_PUBLIC_MEMBERSTACK_PUBLIC_KEY` in `.env` from Dev Tools when you
