...
Everything fictional lives in **`src/lib/site.config.js`**: the name, the headline, and the list
...
| File | What it does |
|---|---|
| `src/lib/site.config.js` | All the demo content. Start here. |
| `src/lib/memberstack.js` | Creates the SDK once, in the browser only. |
| `src/lib/member.svelte.js` | The shared session, as a `$state` module. |
| `src/lib/guard.svelte.js` | `requireMember()` — call it at the top of a gated page. |
| `src/routes/signup`, `src/routes/login` | Email + password, Google, and passwordless. |
| `src/routes/members`, `src/routes/account` | The two gated pages. |
...
**The session is a `$state` object exported from a module**, which is Svelte 5's
shared-state idiom. Note the object is exported and its properties mutated —
reassigning an exported `$state` primitive would not propagate. Call `refresh()`
after anything that changes who is signed in.
...
Pick an adapter. The default `adapter-auto` detects Vercel, Netlify and Cloudflare;
anywhere else needs `adapter-node` or `adapter-static` installed and named in
`svelte.config.js`, or the build warns and produces nothing runnable. A missing
`PUBLIC_` variable fails the build loudly, which is the friendly failure of the six.
