...
> `PUBLIC_MEMBERSTACK_PUBLIC_KEY` out of Vercel's Environment Variables into `.env.local` first.
...
Everything fictional lives in **`src/site.config.js`**: the name, the headline, and the list
...
| File | What it does |
|---|---|
| `src/site.config.js` | All the demo content. Start here. |
| `src/components/memberstack.js` | Creates the SDK. |
| `src/components/useMember.js` | The session hook and `useRequireMember()`. |
| `src/layouts/Layout.astro` | The shell. The nav is the only island in it. |
| `src/pages/index.astro` | The public page. Static except the nav island. |
| `src/pages/members.astro`, `account.astro` | The two gated pages. |
...
**There is deliberately no shared session here.** Astro is multi-page: every
navigation is a full document load, so the nav is rebuilt from scratch each time
and cannot go stale. The React starter needs a context provider to stop the nav
saying "Log in" after signup; that bug cannot happen in this architecture, so
adding cross-island state would be a dependency solving nothing.

The one thing to remember: after signing in or out, navigate with
`window.location` rather than pushing history, so every island on the next page
...
Any static host — the build is plain files. Note that on a static build the HTML of
a gated page is public; only its *contents* are fetched after the session check.
Keep anything genuinely secret out of gated pages and behind `@memberstack/admin`
on a server.

## Before you launch

Test Mode keys start `pk_sb_` and the members you create are free and never
touch your live site. Live keys start `pk_`. Swap `PUBLIC_MEMBERSTACK_PUBLIC_KEY` in `.env.local` from Dev Tools when you
